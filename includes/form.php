	...
	public function form_html( $args = '' ) {
		...
		/*$html = sprintf('<div %s>',
			wpselzy_format_atts(array(
				...
				=> $lang_tag,
				'dir' => wpselzy_is_rtl($this->locale) ? 'rtl' : 'ltr',
			))
		);
...
//        return $html;
	...
	private function form_hidden_fields() {
		$hidden_fields = [
			'_wpselzy'                  => $this->id(),
			'_wpselzy_version'          => WPSELZY_VERSION,
			'_wpselzy_locale'           => $this->locale(),
			'_wpselzy_unit_tag'         => $this->unit_tag(),
			'_wpselzy_container_post'   => 0,
			'_wpselzy_posted_data_hash' => '',
		];

		if ( in_the_loop() ) {
			$hidden_fields['_wpselzy_container_post'] = (int) get_the_ID();
		}
		...
		$hidden_fields += (array) apply_filters(
			'wpselzy_form_hidden_fields', array()
		);
