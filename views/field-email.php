...
<div class="b-selzy-field <?= $isRequired ? 'b-selzy-field_required' : '' ?> b-selzy-form__field" data-selzy-field-name="<?= $name ?>">
    ...
    <input name="<?= $data['field']['slug'] ?>" type="email" id="<?= $data['field']['slug'] ?>" class="b-selzy-field__input" data-selzy-field data-validation-rules="<?= htmlspecialchars(json_encode($data['validations'])) ?>">
